...
export async function isUserSubscribed(event: RequestEvent): Promise<boolean> {
    const result = await getSubscriptionDetails(event);
...
export async function hasAccessToProduct(event: RequestEvent, productId: string): Promise<boolean> {
    const result = await getSubscriptionDetails(event);
    return (
